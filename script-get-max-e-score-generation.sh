...
# ritchie
#root_ids="14 15 16 21 24 28 29 30 37 39 40 41 44 48 49 50 58 6 62 65 66 72 74 75 76 79"
...
do
	...
	generation_max="$f_out/$f_res_revolver/generation_max.$id"
	...
	then
		...
		do
			...
			#gen_hmm_scores="$f_scratch/root.${id}/iteration.$s/generation.hmm.scores" 
        		gen_max="$(~/Dropbox/R.codes/LPOR-max-generation.r $gen_hmm_scores)"
        		echo "GENERATION $s | generation max reached value of $gen_max" >> $generation_max
			#echo "GENERATION $s | generation max reached value of $gen_max"
		done
	fi
done
